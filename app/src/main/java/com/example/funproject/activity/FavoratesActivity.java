...
public class FavoratesActivity extends AppCompatActivity {
private RecyclerView recyclerView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFavoriteHelper = FavoratesDataBaseHelper.getInstance(this);
...
//         使用共享内存中干的aacount,从数据库中获取当前登录用户，用于后续查询
        ...
        //       从数据库中获取favorite
        List<Favorites> mFavorites = mFavoriteHelper.queryByUid(mUser);
//        //从数据库中获取Video
        for(int i=0;i<mFavorites.size();i++){
            mVideoes.add(mVideoHelper.queryByVId(mFavorites.get(i).getVid()));
        }
    ...
    private void initView() {
        recyclerView = findViewById(R.id.ry_favorites);
        //设置布局显示分隔线
